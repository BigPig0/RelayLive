...
namespace Server
{
    ...
    class CLiveSession : public live_session
    {
    public:
        CLiveSession();
        ~CLiveSession();
        ...
        string                strCode;              //uri中解析出参数 设备编码
        string                strType;              //uri中解析出参数 媒体类型
        string                strHw;                //uri中解析出参数 视频大小

		CNetStreamMaker       wsBuff;
    ...
    void CLiveSession::OnRecv(char* data, int len) {
        ...
        if(!parseHeader && dataCatch.find("\r\n\r\n") != std::string::npos) {
            ...
            pWorker = CreatLiveWorker(strCode, strType, strHw, isWs, this, clientIP);
