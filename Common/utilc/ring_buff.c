...
size_t ring_consume(struct ring_buff *ring, uint32_t *tail, void *dest,
		 size_t max_count)
{
	uint8_t *odest = dest;
	void *orig_tail = tail;
	...
	n = (int)(ring_get_count_waiting_elements(ring, tail) *
							ring->element_len);
	...
	if (!dest) {
		*tail = ((*tail) + n) % ring->buflen;
		if (!orig_tail) /* single tail */
			ring_update_oldest_tail(ring, *tail);
...
void ring_dump(struct ring_buff *ring, uint32_t *tail)
{
	...
	printf("ring %p: buflen %u, elem_len %u, head %u, oldest_tail %u\n"
		    ...
		    ring->oldest_tail,
		    (int)ring_get_count_free_elements(ring), *tail,
		    (int)ring_get_count_waiting_elements(ring, tail));
}
