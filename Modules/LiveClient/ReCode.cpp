...
static void H264SpsCbfun(uint32_t nWidth, uint32_t nHeight, double fFps, void* pUser){
    CODECTOOL* pLive = (CODECTOOL*)pUser;
...
CReCode::CReCode(void* handle)
    ...
    , m_tick_gap(3600)
{
    m_decode = new DECODETOOL;
    m_codec = new CODECTOOL;
...
CReCode::~CReCode(void)
{
    Log::debug("CReCode release");
}

void CReCode::SetChannel1(uint32_t nWidth, uint32_t nHeight, AV_CALLBACK cb)
{
...
int CReCode::ReCode(AV_BUFF buff, NalType t)
{
    if(!m_decode->ok){
    ...
    case b_Nal:  // 非关键帧
        {
            ...
            MakeVideo(buff.pData, buff.nLen,0);
...
bool CReCode::MakeVideo(char *data,int size,int bIsKeyFrame)
{
	/*
