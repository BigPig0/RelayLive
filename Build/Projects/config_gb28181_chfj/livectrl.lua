function GetDevInfo()
    ...
    LUDB_EXECUTE_STMT(stmt, "select t.dev_guid, t.dev_name, t.STATUS, t.LATITUDE, t.LONGITUDE from GB28181_DEVICE t where t.dev_source = 1")
...
function UpdateStatus(code, status)
    ...
    local sql = string.format("update GB28181_DEVICE set STATUS = %d, updatetime = to_date('%s','yyyymmddhh24miss') where dev_source = 1 and dev_guid = '%s'", sta, date, code)
    print(sql)
    local stmt = LUDB_CREATE_STMT(con)
...
function UpdatePos(code, lat, lon)
    ...
    local date=os.date("%Y%m%d%H%M%S")
    local sql = string.format("update GB28181_DEVICE set LATITUDE = %s, LONGITUDE = %s, updatetime = to_date('%s','yyyymmddhh24miss') where dev_source = 1 and dev_guid = '%s'", lat, lon, date, code)
    print(sql)
    local stmt = LUDB_CREATE_STMT(con)
    LUDB_EXECUTE_STMT(stmt, sql)
    LUDB_COMMIT(con)
...
function Init()
    ...
    --设备表插入工具
    local sql = "insert into GB28181_DEVICE (dev_guid, dev_name, LATITUDE, LONGITUDE, STATUS, dev_source) values (:GUID, :NAME, :LAT, :LON, :STATUS, 1)"
